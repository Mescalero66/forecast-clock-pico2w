...
def calc_cpu_temp():
    reading = cpu_temp.read_u16() * (3.3 / 65535)
    cpu_temperature = 27 - (reading - 0.706) / 0.001721
...
async def system_setup():
    ...
    disp8.set_string(temp_string, "r")
    await asyncio.sleep(0.5)
    # calculate day_of_week, required to set the internal clock (UTC)
    ...
    # set the clock time from the converted local time from TimezoneInfo
...
async def synchronise_watches(_last_sync_time, _call_time):
    try:
        ...
        day_of_week = TimeCruncher.get_weekday(tziData.local_year, tziData.local_month, tziData.local_day)
        clock_new = TimeCruncher.now_rtc_to_epoch((tziData.local_year, tziData.local_month, tziData.local_day, day_of_week, tziData.local_hour, tziData.local_minute, tziData.local_second, 0))
        await asyncio.sleep(0.2)
        # calculate what the difference is, guard against huge jumps
        clock_diff = clock_new - clock_current
        clock_adj_thresh = (_call_time - _last_sync_time) * WATCH_SYNC_ADJUST_MAX_PERCENT
        clock_adj_thresh = max(1, int(clock_adj_thresh))
...
async def update_display_clock():
    while True:
        _, _, _, _, hh, mm, ss, _ = rtc.datetime()
...
async def update_forecast(_last_sync_time, _forecastMeta, _forecastData, _timezoneOffset):
    ...
    lclY, lclM, lclD, _, _, _, _, _ = rtc.datetime()
